...
content_area <- attr(math, "content_area")
record_type <- attr(math, "record_type")
data_source <- attr(math, "data_source")
num_students <- format(length(unique(math$ID)), big.mark = ",")
...
student_population <- summarize_students_by_grade_year(dataset = math)
...
cohort1 <- analyze_student_cohorts(dataset = math)
