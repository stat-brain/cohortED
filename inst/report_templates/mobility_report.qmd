...
change = plot_distribution_mobility(dataset = math, year = "2022_2023", start_grade = 4, end_grade = 6, print_table = FALSE, data_out = TRUE, print_plot = FALSE)

new.table = xtabs(PERCENT ~ MOBILITY + GRADE, data = change)
colnames(new.table) = c("Grade 3", "Grade 4", "Grade 5")
new.table = apply(new.table, c(1, 2), function(x) {
  sprintf("%.1f%%", x)  # formats with 2 decimal places and adds '%'
})

knitr::kable(new.table, caption = "Mobility Status by Grade for 2023-2024", align = rep("r", ncol(new.table)))
```
...
grade4 = plot_alluvial_mobility(dataset = math, 
                                start_year = "2022_2023", 
                                start_grade = 4, print_table = FALSE, data_out = TRUE)
```
...
mobility_ethnicity = round(prop.table(table(grade4$MOBILITY_STATUS, 
                                            grade4$ETHNICITY), margin = 2), 3) * 100
mobility_ethnicity = apply(mobility_ethnicity, c(1, 2), function(x) {
  sprintf("%.1f%%", x)  # formats with 2 decimal places and adds '%'
})

knitr::kable(mobility_ethnicity, align = rep("r", ncol(mobility_ethnicity)))
